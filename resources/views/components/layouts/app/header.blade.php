...
<body class="min-h-screen bg-white dark:bg-zinc-800">
    <flux:header container class="border-b border-zinc-200 bg-zinc-50 dark:border-zinc-700 dark:bg-zinc-900">
        ...
        <flux:navbar class="-mb-px max-lg:hidden">
            ...
            <flux:navbar.item icon="arrow-path" :href="route('raffles')" :current="request()->routeIs('raffles')"
                wire:navigate>
                {{ __('Raffles') }}
            </flux:navbar.item>
            <flux:navbar.item icon="trophy" :href="route('events')" :current="request()->routeIs('events')"
                wire:navigate>
                {{ __('Events') }}
            </flux:navbar.item>
            <flux:navbar.item icon="swatch" :href="route('tickets')" :current="request()->routeIs('tickets')"
                wire:navigate>
                {{ __('My Tickets') }}
            </flux:navbar.item>
        </flux:navbar>
    ...
    <flux:sidebar stashable sticky
        ...
        <flux:navlist variant="outline">
            <flux:navlist.group :heading="__('Platform')">
                ...
                <flux:navlist.item icon="arrow-path" :href="route('raffles')" :current="request()->routeIs('raffles')"
                    wire:navigate>
                    {{ __('Raffles') }}
                </flux:navlist.item>
