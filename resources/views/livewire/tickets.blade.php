<div class="w-full">
    ...
    <div class="relative mb-8 overflow-hidden rounded-xl bg-gradient-to-r from-blue-600 to-purple-600 p-8 shadow-lg">
        <div class="relative z-10">
            <flux:heading size="xl" level="1" class="text-white">{{ __('My Tickets') }}</flux:heading>
            <flux:subheading size="lg" class="text-blue-100">{{ __('My lottery tickets') }}</flux:subheading>
        </div>
    ...
    <div class="mb-6">
        <flux:modal.trigger name="create-ticket">
            <button class="group relative overflow-hidden rounded-xl bg-gradient-to-r from-blue-600 to-purple-600 px-8 py-4 text-left shadow-lg transition-all hover:shadow-xl hover:scale-105">
                ...
                <div class="absolute inset-0 bg-gradient-to-r from-blue-700 to-purple-700 opacity-0 transition-opacity group-hover:opacity-100"></div>
            </button>
        </flux:modal.trigger>
    </div>
                ...
                @foreach ($tickets as $ticket)
                    <flux:table.row :key="$ticket->id">
                        ...
                        <flux:table.cell align="center">
                            <span class="font-mono text-sm font-semibold text-gray-700 dark:text-gray-300">{{ $ticket->code }}</span>
                        </flux:table.cell>

                        <flux:table.cell align="center">
                            <span class="inline-flex items-center justify-center rounded-full bg-gradient-to-r from-blue-100 to-purple-100 px-3 py-1 text-sm font-semibold text-blue-700 dark:from-blue-950/20 dark:to-purple-950/20 dark:text-blue-400">
                                {{ $ticket->ticketDetails->count() }}
                            </span>
                        </flux:table.cell>

                        <flux:table.cell align="end">
                            <span class="font-semibold text-gray-900 dark:text-white">${{ number_format($ticket->stake_amount, 2) }}</span>
                        </flux:table.cell>
                        ...
                        <flux:table.cell align="center">
                            @if($ticket->won)
                                <div class="inline-flex items-center gap-1 rounded-full bg-gradient-to-r from-green-100 to-emerald-100 px-3 py-1 text-sm font-semibold text-green-700 dark:from-green-950/20 dark:to-emerald-950/20 dark:text-green-400">
                        ...
                        <flux:table.cell align="center">
                            @if($ticket->prize)
                                <span class="font-bold text-green-600 dark:text-green-400">${{ number_format($ticket->prize, 2) }}</span>
                            @else
